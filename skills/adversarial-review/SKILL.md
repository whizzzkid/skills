...
| 2.2 | Changed script/module/parallel pipeline | List directory siblings and whole-repo sibling toolchain invocations. A directive (`soft_fail`, `retry`, `timeout`, exit-code handling) copied from a sibling → verify the sibling's behavioral/exit-code contract actually transfers (pattern copy ≠ contract transfer). | Blocker | Apply the fix to every sibling or justify absence; a copied directive whose contract does not hold — quote the sibling's contract or flag pending verification. |
...
| 2.10 | Existing PR | Fetch title/body; check behavior wording, test counts, file lists, remaining work, metadata, Jira suffix, rename/enum drift, rollout/ops section for prod-facing diffs. Enum-like body lists (reason symbols, tags, flags, error codes): grep post-diff code for all values; any missing from the body is drift. | Blocker | Record body drift as post-push TODO; fix before marking ready. |
...
| 2.19a | Added Struct/Record/interface/Go field | Grep tests for direct concrete-value assertion on the new field. When the field is serialized via `.to_s`/equivalent, also include a nil/false/0 case — the zero-value path through a serialization boundary is the common production path (silent skips, error rescues) and a `NoMethodError` there escapes happy-path specs. | Blocker | Add direct assertion; `respond_to?`/presence alone is insufficient; add the nil/zero-value serialization case. |
| 2.20 | Application code + CI pipeline | Extract net-new env reads; locate invoking pipeline steps; verify allowlist forwarding. Diff touches a compose `environment:` or plugin `env:` → full-path audit: grep every script AND library in the container's runtime call graph (not just the diff delta) for env reads, diff that set against the forwarding list, run a sibling-template consistency check. | Blocker | Forward vars in native/container steps; exempt auto-injected prefixes only for native non-container steps. Surface any runtime read without a forwarding entry (incl. pre-existing reads in called libs); same-role siblings forward the same logical set. A spec deleting a var and asserting the script's own default = intentional omission → downgrade the missing-passthrough finding to `question`. |
...
| 2.34 | Spec/doc routing claim (which method a gate calls, which path bypasses a hook) | Grep the PR review thread for reviewer statements describing the same routing. | Blocker | A reviewer who read the source is ground truth; resolve contradictions before asserting an inferred claim. |
...
| 2.37 | Gate reorder moves a cheap guard before a deeper call | Reordering short-circuits a failure path earlier → calls it used to make become unreachable. Enumerate every now-unreachable call; check each `not_to receive` test covers all, not just the deepest (the diff usually touches only the deepest). | Suggestion | Add the missing negative assertions; escalate to Blocker when a now-unreachable call was previously unstubbed and would hit the network/a real dependency. |
...
| 2.43 | New field beside an existing same-primitive-type field | Grep for a resolver/normalizer/sanitizer on the sibling (`resolve*`/`normalize*`/`sanitize*`, often at load time); confirm the new field gets equivalent treatment (a raw new field skips it silently). | Blocker | Apply the same normalizer. Blocker when the field feeds a security-sensitive consumer (paths, URLs, shell args, allow-dir lists) — a raw path/URL field is a traversal/SSRF vector. |
| 2.44 | Merge/rebase conflict resolved at a function call site | Compare both sides' arg counts against the current base-branch signature; the base is authoritative for required params (a side missing one is stale, not a "which caller wins" choice). Also diff both sides for signal/context/cleanup primitives (`signal.Stop`, `context.Cancel*`, `sync.*`, `defer`, `close(`, `os.RemoveAll`, resource releases) on the base/HEAD side but absent from the result — base state is canonical, so a missing guard is a dropped safety contract, not a caller choice. | Blocker | Take the side matching the base signature; flag the short call as a defect. Restore any base-side safety primitive missing from the result unless the incoming commit removed it with rationale; green tests do not prove it unneeded. |
...
- **Runtime-behavior-cautious:** any claim about how a tool behaves under failure (exit codes, signal handling, `--write-out`/buffering, subshell/pipe semantics) is at most `question` with a repro request — never `blocker` from first principles. Tool behavior is verified in the playground (Step 5), not deduced.
- **Absence-claim-cautious:** a finding that a "safe no-op" or missing error-path write is a defect must cite a concrete failure scenario where the absence causes wrong behavior. Absence of defensive code is not itself a defect — writing a default (e.g. `{}`) on read failure can clobber legitimate local-only state when the no-op is the correct bootstrap. Cap at `question` without a repro.

### Categories to Hunt

`category:` (Step 4) is one of, sweep catalog holds the detail: **Logic / arithmetic** (off-by-one, first/last/one-past-end, pagination edges); **Type coercion** (`"0"` vs `0` vs `false`, `[]` vs `{}`, semantic violations); **State / ordering / concurrency** (use-before-init, use-after-close, double-init, async interleave, races, lock asymmetry); **Contract / cross-system** (signature widening, consumer sweep, producer≠consumer layout, recursion mismatch, cleanup-before-verify); **Refactor-removed** (validation, recursion, error handling silently dropped); **Test quality** (tautology, missing assertions/failure path, asymmetric field coverage); **Security / data loss** (injection, secret leakage, redaction gaps, traversal, unprotected writes, missing rollback); **Error handling** (swallowed errors, generic catches, wrong error class); **Runtime / performance** (shell/runtime matrix gaps, quadratic scans, missing indexes, repeated I/O).
...
file persistence), or every changed file is docs/prose/fixture data (read-based
pass, no scratch scripts), apply the matching block from
[`references/playground-specialized-checks.md`](references/playground-specialized-checks.md).
...
PR-body-only blocker (sweep 2.8/2.10 body drift, no code change) → fix via `gh pr edit`, no new commit; re-verify against the same HEAD SHA. The `.cleared-{HEAD_SHA}.json` record stays valid (code unchanged); a no-op commit would pollute history.
